...
- `feedbackUrl`：外部反馈表单的 HTTP(S) 链接。
- `emergencyQQ`：联系 QQ 号，保留字符串形式。

QQ、预约和反馈链接是公开配置，管理密码只保存在 Cloudflare Secret 中。所有页面通过 `data-config-link` 声明配置键，`assets/app.js` 统一读取 `/config.js` 并绑定地址；留空或无效时只禁用对应入口并显示提示。右下角联系入口使用同一份配置。

兼容旧后台记录：仅缺少 `feedbackUrl` 时读取源码默认值；已保存的自定义地址或空字符串不会被覆盖。保存接口要求提交全部三个字段，旧管理页需刷新后再保存。
...
- 默认表单地址只维护在 `config.js`，不再写入 HTML。未启用 JavaScript 时显示启用提示，避免备用地址与配置不一致。
